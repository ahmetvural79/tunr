// Package relayx contains the new "cloud" upstream for the tunr relay.
//
// Today the relay maps  subdomain -> live tunnel session (WebSocket to CLI).
// This file adds a second upstream kind:  subdomain -> CloudUpstream, which
// reverse-proxies to an app container/machine and knows how to WAKE it.
//
...
// Waker abstracts "make this app dialable". DockerDriver implements it on the
// own-server path; on Fly it's a no-op (Fly Proxy wakes machines itself).
type Waker interface {
	Wake(ctx context.Context, appID string) error
}

// FreezeCache is implemented by the existing freeze-mode store: last good
// response per (host, path) — reused here to mask redeploy gaps and wake
// failures. Wire the real implementation in.
...
type CloudUpstream struct {
	AppID       string
	Target      *url.URL      // http://172.20.0.7:8080 or http://tunr-a-x.flycast:80
	EdgeSecret  []byte        // HMAC key; tunr-shim in the container verifies
	WakeTimeout time.Duration // total budget for probe+wake, e.g. 30s
	Waker       Waker
	Freeze      FreezeCache // optional

	initOnce sync.Once
	proxy    *httputil.ReverseProxy
	// lastSeen feeds the idle sweeper (control plane reads it to Sleep apps).
...
func (u *CloudUpstream) init() {
	u.proxy = &httputil.ReverseProxy{
		...
		// TODO: ModifyResponse -> u.Freeze.Store(...) for cacheable GET 200s
...
// ---------- Route store (subdomain -> upstream) ----------

// RouteKind mirrors the routes.kind column: tunnel | cloud | hybrid.
// v0 resolves tunnel (existing in-memory path) first, then cloud.
// "hybrid" (tunnel if connected, else cloud) is the future `tunr detach`.
...
// Invalidation: subscribe to Postgres `LISTEN routes_changed` (payload =
// subdomain), re-read the row and SetCloud/Delete accordingly; fall back to a
// 10s full re-poll if the LISTEN connection drops. Wire with jackc/pgx:
//
//   conn.Exec(ctx, "LISTEN routes_changed")
//   for { n, _ := conn.WaitForNotification(ctx); store.reload(n.Payload) }
//
// Integration point in the existing relay router:
//
//   if sess := tunnels.Lookup(sub); sess != nil { sess.ServeHTTP(w, r); return }
//   if up, ok := routes.LookupCloud(sub); ok   { up.ServeHTTP(w, r);  return }
//   http.NotFound(w, r)
